...
public class ALPR {
    ...
    private void executeRecognition(final Mat m, final String country, final ResultsCallback callback, final WeakReference<Context> context, int rotation) {
        Log.i(TAG, "executeRecognition country: "+country+" rotation: "+rotation);
        ...
        Log.i(TAG, "executeRecognition openAlprConfFile: "+openAlprConfFile+" file: "+file.getAbsolutePath()+" result: "+result);
                ...
                public void run() {
                    if (results == null || results.getResults() == null || results.getResults().size() == 0) {
                        ...
                        if(results!=null && results.getResults() != null){
                            Log.d(TAG, "results.getResults() != null");
                        }
                        if(results!=null && results.getResults() != null && results.getResults().size() != 0){
                            Log.d(TAG, "results.getResults().size() != 0");
                        }
                        callback.onFail();
                    } else {
